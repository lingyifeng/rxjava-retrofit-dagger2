...
import com.example.video.utils.AnimHelper;
import com.example.video.utils.MotionEventUtils;
import com.example.video.utils.NetWorkUtils;
import com.example.video.utils.SDCardUtils;
import com.example.video.utils.StringUtils;
import com.example.video.utils.WindowUtils;
import com.example.video.widgets.MarqueeTextView;
import com.example.video.widgets.ShareDialog;
...
import static com.example.video.utils.StringUtils.generateTime;
...
public class IjkPlayerView extends FrameLayout implements View.OnClickListener {
    ...
    private LinearLayout mLySeeker;
...
//    private TextView mTvRecoverScreen;
    ...
    //
    private boolean isNeedWindowTop = false;
    ...
    private void _initView(Context context) {
        ...
        mLySeeker = (LinearLayout) findViewById(R.id.lySeeker);
...
//        mTvRecoverScreen.setOnClickListener(this);
     ...
     * 设置 播放类型
     */
    public IjkPlayerView setIsLiving(boolean flag) {
        mIsLiving = flag;
        mLySeeker.setVisibility(mIsLiving ? INVISIBLE : VISIBLE);
        return this;
    }
...
//                    mTvRecoverScreen.setVisibility(isShowBar ? View.VISIBLE : View.GONE);
                ...
                mWindowTopBar.setVisibility((isNeedWindowTop && isShowBar) ? View.VISIBLE : View.GONE);
                mFullscreenTopBar.setVisibility(View.GONE);
                mIvPlayerLock.setVisibility(View.GONE);
                if (mIsNeedRecoverScreen) {
...
//                mTvRecoverScreen.setVisibility(VISIBLE);
    ...
    private void _toggleMediaQuality() {
        ...
        if (mIsShowQuality) {
            ViewCompat.animate(mFlMediaQuality).translationX(mFlMediaQuality.getWidth()).setDuration
                    (DEFAULT_QUALITY_TIME);
...
//                mTvRecoverScreen.setVisibility(GONE);
    ...
    private GestureDetector.OnGestureListener mPlayerGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            if (!mIsForbidTouch && !mIsNeverPlay && !mIsLiving) {
        ...
        public boolean onSingleTapConfirmed(MotionEvent e) {
            // 弹幕编辑状态返回则不执行单击操作
            if (isRecoverFromDanmaku || mIsLiving) {
                return true;
            }
            if (mIsShowQuality) {
                _toggleMediaQuality();
...
//        mVideoView.setOnCompletionListener(l);
     ...
     * @param mediaBd     1080P
     */
    public IjkPlayerView switchVideoSource(String mediaSmooth, String mediaMedium, String mediaHigh, String
            mediaSuper, String mediaBd) {
        reset();
     ...
     * @param mediaBd     1080P
     */
    public IjkPlayerView setVideoSource(String mediaSmooth, String mediaMedium, String mediaHigh, String mediaSuper,
                                        String mediaBd) {
        boolean isSelect = true;
        mQualityData = new ArrayList<>();
        if (mediaSmooth != null) {
            mVideoSource.put(MEDIA_QUALITY_SMOOTH, mediaSmooth);
            mQualityData.add(new MediaQualityInfo(MEDIA_QUALITY_SMOOTH, mMediaQualityDesc[MEDIA_QUALITY_SMOOTH],
                    isSelect));
        ...
        if (mediaMedium != null) {
            ...
            mQualityData.add(new MediaQualityInfo(MEDIA_QUALITY_MEDIUM, mMediaQualityDesc[MEDIA_QUALITY_MEDIUM],
                    isSelect));
            if (isSelect) {
        ...
        if (mediaSuper != null) {
            ...
            mQualityData.add(new MediaQualityInfo(MEDIA_QUALITY_SUPER, mMediaQualityDesc[MEDIA_QUALITY_SUPER],
                    isSelect));
            if (isSelect) {
